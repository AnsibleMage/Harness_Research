<!-- Ann's Global CLAUDE.md v1.0 | 2026-04-03 | Harness Engineering 기반 설계 -->
...
1. **Explore** (Plan Mode): 코드 읽기, 요구사항 파악, 변경하지 않음
2. **Plan** (Plan Mode): 상세 구현 계획 작성, 아키텍처/설계 결정
3. **Implement** (Normal Mode): 코드 작성, 테스트 실행, 수정
4. **Commit**: 커밋 메시지 작성, PR 생성, 문서화

## 복잡도별 접근

- **단순 작업** (단일 파일, 명확한 범위): 즉시 실행
- **중간 복잡도** (2~5 파일, 단계 필요): 단계별 분석 후 실행
- **높은 복잡도** (다중 파일, 아키텍처 영향): 분해 → 분석 → 통합 → 검증
...
- 코드베이스 조사, 보안 리뷰 등 탐색 작업은 subagent에 위임하여 메인 context 보호
- 구현 후 검증도 subagent에 위임 가능: "use a subagent to review this code"

## Skills 활용

- 반복되는 워크플로우는 `.claude/skills/`에 SKILL.md로 정의하여 재사용
- 도메인 지식이나 특정 절차는 Skills에 넣어 on-demand 로딩 (CLAUDE.md에 넣지 않음)
...
<!-- 강제 실행이 필요한 보안 규칙은 Hooks/Permissions(.claude/settings.json)으로 분리 권장 -->
