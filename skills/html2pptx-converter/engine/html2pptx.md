...
Every HTML slide must include proper body dimensions. Standard layouts use fixed sizes; CUSTOM layouts use defineLayout().
...
- **CUSTOM** (wireframe): Any positive pt values, requires `pres.defineLayout()` in convert.js

### Wireframe Mode

와이어프레임 모드에서는 표준 레이아웃 대신 커스텀 body 크기를 사용한다.

**지원 색상** (4가지만 허용):
- `#000000` — 텍스트, 테두리, 배지 배경
- `#808080` — 강조 배경 (버튼, 배너)
- `#E0E0E0` — 섹션 배경, 비활성 영역
- `#FFFFFF` — 카드 배경, 콘텐츠, 배지 텍스트

**body 크기**: 커스텀 가능 (defineLayout 필요)
```javascript
// convert.js에서 CUSTOM 레이아웃 설정
...
기존 HTML 규칙(display: flex, 텍스트 태그, web-safe 폰트 등)은 동일하게 적용된다.
...
### Element Rendering Order (오버레이 필수 규칙)

html2pptx는 HTML 요소를 **문서 순서(DOM order)**대로 PPTX 슬라이드에 배치한다.
- 먼저 나오는 요소 = 아래 레이어 (다른 요소에 덮임)
- 나중에 나오는 요소 = 위 레이어 (다른 요소 위에 표시)

따라서 **오버레이 요소**(배지, 마커, 워터마크 등)는 반드시 HTML body의 **마지막**에 배치해야 한다.

```html
<body>
  <!-- 1. 콘텐츠 섹션 먼저 (아래 레이어) -->
  <div style="position:absolute; ...">섹션 내용</div>

  <!-- 2. 오버레이 요소는 항상 마지막 (위 레이어) -->
  <div style="position:absolute; z-index:9999; ...">마커/배지</div>
</body>
```

**주의**: CSS z-index만으로는 불충분하다 — **DOM 순서가 PPTX 렌더링 순서를 결정**한다. 오버레이 요소가 콘텐츠보다 앞에 있으면 콘텐츠에 덮여 보이지 않는다.
